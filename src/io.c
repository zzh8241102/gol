...
int file_parser_init(char *df, game_state *game)
{
    ...
    if (sscanf(df, "grid_size = %s", game->grid_notion) == 1)
    {
        ...
        else
        {
            ...
            printf("config error, will use default\n");
        }
        return 0;
    }
    if (game->is_inited == 1)
    {
        if (sscanf(df, "x,y = %d,%d", &game->init_x, &game->init_y) == 2)
        {
            ...
            {
                printf("invaild input, which will have no effects on the grid.\n");
            }
            else
            {
                *(*(game->grid + game->init_x) + game->init_y) = 1;
            }
        }
        return 0;
    }
...
// Advanced listen to the current grid_created by the click_operation

// write the final result to file
int write_result(FILE *file, game_state *game)
{
}